...
HTTP_TIMEOUT = float(os.getenv("HTTP_TIMEOUT", "12.0"))  # timeout aumentado
...
_snapshot_cache = {}  # cache de snapshots

# ============== Funções auxiliares ==============
def _binance_get(path: str, params: dict):
    """Wrapper robusto para GET com timeout e log de erro."""
    ...
    try:
        ...
        print(f"[ERROR] _binance_get: {url} -> {e}")
...
def fetch_agg_trades_window(symbol: str, start_ms: int, end_ms: int, per_req: int = AGG_LIMIT) -> list:
    ...
    while True:
        ...
        if safety > 50:
            break
        r = requests.get(f"{BINANCE_BASE}/aggTrades", params=params, timeout=HTTP_TIMEOUT)
...
def m5_snapshot():
    ...
    payload = {
        ...
        "guards": guards,
        "meta": {"data_version": "snap_1.1.3", "notes": "timeout fix + fapi endpoint"}
    }
